...
export default function Page() {
  ...
  const filtered = useMemo(
    () => entries.filter((item) => item.type === "weekly-reflection"),
    [entries]
  );
  ...
  return (
    <section className="py-10 sm:py-14">
      ...
      <h1 className="mt-4 text-3xl font-semibold">Weekly Reflection</h1>
      <p className="mt-3 max-w-3xl text-white/70">
        This page stores weekly reflections so the user can understand what worked, what failed, and what should improve next week.
      </p>
      ...
      <div className="mt-8 space-y-4">
        {filtered.length ? (
          ...
          <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-white/70">
            No weekly reflection entries yet.
          </div>
        )}
      </div>
    </section>
